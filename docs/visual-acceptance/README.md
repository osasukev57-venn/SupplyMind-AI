...
- 真实本地应用逐页渲染 7 个页面；最终截图由 Chrome headless 在隔离 dataRoot 上重新生成，不使用静态 HTML
...
- 全页面治理文本回归零命中；governance.spec 3 用例通过
...
- `Manual`/`LocalImport`/`SyntheticDemo` 与 provider id 只保留在 wire/配置值中，来源名称通过 `sourceDisplayName` 产品化展示
...
- History/Quality 的证据异常只展示稳定中文状态与处理含义，不直接泄漏英文文件/manifest/decode 实现消息

【Navigation IA】PASS
- 左侧导航三分组：监测（总览/历史趋势/数据质量）、数据管理（来源与录入/动态配置）、运营分析（预警/Agent 工作台）
- 顶栏仅保留页面标题与状态（无冗余操作）
- 768px 折叠为横向滚动导航条（见 dashboard-768.png）
- 运营宽表在 768px 保持可读列宽并横向滚动，不再压缩成不可辨识的细列（见 config-768.png、sources-768.png）
...
| 页面 | 1440×1000 light | 1440×1000 dark | 768×1024 responsive |
|---|---|---|---|
...
6. 所有现有业务字段与功能仍可访问：33 个前端用例 + production build + 真实页面渲染均通过，未删除任何功能
