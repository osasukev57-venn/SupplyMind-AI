...
|---|---|
| 当前开发日 | Day 2 |
| 当前任务编号 | D3-T05 LocalImport与SyntheticDemo隔离（`TaskExecutionStatus=REVIEW_PENDING`，`statusReason=D3T05_XLSX_ITEM_RAW_BYTES_FIXED_20260810`；R1+ 首审=CHANGES_REQUESTED，第二次 Delta Review 唯一 MAJOR（XLSX item raw payload）已修复，等待第二方 R1+ 复审）。 |
| 当前任务状态 | D1-T01～D1-T05、D2-T01～D2-T05、D3-T01、D3-T02、D3-T03、D3-T04 均为`TaskExecutionStatus=DONE`；Day 1=`COMPLETE`、Day 2=`COMPLETE`、AT-SRC-002=`PASS`、DEC-050～056 已生效（DEC-056 implementation=`PASS`）；DEC-057=`EFFECTIVE`。D3-T05：CSV 行 payload=精确原始字节 span（PASS 保持）；XLSX item raw payload 已改为 ORIGINAL_FULL_FILE_BYTES（item payloadSha256==SHA256(原始XLSX文件)==source import SHA；不落 derived cell JSON；acquisitionId/importId 稳定追溯同一 source import；行业务信息为结构化/derived 证据）；XLSX 幂等按结构化字段比较；Synthetic 隔离/Partial Import/Publish Gate 不变。D3-T06=`NOT_STARTED`。 |
...
| 最近一次可运行版本 | backend：Java 17 + Spring Boot 3.3.6；全套 234 项测试（43 classes，0 failures，0 errors，7 skipped 门禁）通过（D3-T05 XLSX item raw 修复后最新全量回归；未重跑真实联网 AT-SRC-002——未改动其正式数据语义）。 |
| 最近一次Git提交 | 本轮将提交 `fix: preserve XLSX item raw bytes`；前序 checkpoint=`f4dc00f`（D3-T05 import raw handling，第二次 Delta Review 唯一 MAJOR）。 |
| 是否偏离计划 | 否 |
| 最后更新人/窗口 | OpenCode实施工程师窗口，D3-T05 Final Finding Fix（第二次 Delta Review 唯一 MAJOR）：XLSX item 级 RawReceiptV1.payload 由 derived cell-fact JSON 修正为 ORIGINAL_FULL_FILE_BYTES（docs/01 L354 完整字节语义）——item payloadSha256==SHA256(originalXlsxBytes)==source import SHA，且 != SHA256(derived JSON)（测试断言）；行业务信息（含声明来源）保留为结构化/derived 证据，不冒充 raw payload；acquisitionId/importId 稳定追溯同一 source import；XLSX 幂等按结构化字段比较（importId/时间不参与）；CSV span 语义/quoted-newline/CRLF/XLSX 解析与精度/Raw-first/Malformed 保留/Idempotency/Synthetic 隔离/Publish Gate 全部保持；D3-T05=`REVIEW_PENDING`（等待第二方 R1+ 复审）、Day 3=`NOT_COMPLETE`；未开始 D3-T06。 |
