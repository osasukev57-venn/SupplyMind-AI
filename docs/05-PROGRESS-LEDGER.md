...
|---|---|
| 当前开发日 | Day 2 |
| 当前任务编号 | D3-T05 LocalImport与SyntheticDemo隔离（`TaskExecutionStatus=REVIEW_PENDING`，`statusReason=D3T05_IMPORT_RAW_HANDLING_COMPLETED_20260810`；R1+ 首审=CHANGES_REQUESTED，fix 后等待第二方 R1+ 复审）。 |
| 当前任务状态 | D1-T01～D1-T05、D2-T01～D2-T05、D3-T01、D3-T02、D3-T03、D3-T04 均为`TaskExecutionStatus=DONE`；Day 1=`COMPLETE`、Day 2=`COMPLETE`、AT-SRC-002=`PASS`、DEC-050～056 已生效（DEC-056 implementation=`PASS`）；DEC-057=`EFFECTIVE`。D3-T05 R1+ Finding Fix 完成：XLSX 补齐（POI，文本单元格精确 decimal/非文本拒绝）、file-level raw-first（`LocalImportReceiptV1` 完整原始文件 bytes 先于解析持久化，失败也保留）、row-level raw=精确原始字节 span（quoted-newline/UTF-8 多字节/CRLF 保留）、CRLF 合法化；Partial Import/幂等/Synthetic 隔离不变。D3-T06=`NOT_STARTED`。 |
...
| 最近一次可运行版本 | backend：Java 17 + Spring Boot 3.3.6；全套 233 项测试（43 classes，0 failures，0 errors，7 skipped 门禁）通过（D3-T05 finding fix 后最新全量回归；未重跑真实联网 AT-SRC-002——未改动其正式数据语义）。 |
| 最近一次Git提交 | 本轮将提交 `fix: complete D3-T05 import raw handling`；前序 checkpoint=`0e8165c`（D3-T05 implementation，R1+ 首审=CHANGES_REQUESTED）。 |
| 是否偏离计划 | 否 |
| 最后更新人/窗口 | OpenCode实施工程师窗口，D3-T05 R1+ Finding Fix：MAJOR1 XLSX 补齐（Apache POI 5.2.5；XLSX 文本单元格精确 decimal、非文本单元格文件级拒绝 NON_TEXT_CELL 防 double 污染；字段语义与 CSV 同一 row model）；MAJOR2 file-level raw-first（`LocalImportReceiptV1`+`LocalImportFileStore`：完整原始文件 bytes 先于任何解析持久化+COMMITTED manifest，CSV 非法 UTF-8/畸形引号/错误 header/BOM 与 XLSX 损坏/错误 header/schema 等全部失败均保留源证据）；MAJOR3 row-level raw payload=逻辑记录精确原始字节 span（引号内换行/UTF-8 多字节/quoted comma/escaped quote/CRLF 原样）；普通问题 CRLF 合法化（不再因 \r\n 全文件拒绝，raw 保留原始 CRLF；BOM 拒绝保持冻结）；Partial Import/幂等/SyntheticDemo 隔离/Publish Gate 全部保持；D3-T05=`REVIEW_PENDING`（等待第二方 R1+ 复审）、Day 3=`NOT_COMPLETE`；未开始 D3-T06。 |
