...
打开「总览」，每张卡片显示：最新值、单位、业务日期、数据来源、采集路线、完整率与校验版本。桌面启动后会后台获取人民银行最新公开公告，顶部显示采集状态；完成后默认 USD/CNY、EUR/CNY 卡片自动刷新，也可手动重试。“最新”指人民银行最近已公布业务日的中间价，不是流式行情。「暂无数据」表示尚无已发布（PUBLISHED + VERIFIED 类）数据。
