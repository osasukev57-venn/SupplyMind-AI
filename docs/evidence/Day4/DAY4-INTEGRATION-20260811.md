...
>
> 【R2 Stage Fix 追加】（2026-08-11，`fix: close Day4 stage review findings`）：Candidate `a9fe94a` Sol Stage Review=`CHANGES_REQUESTED`（4×MAJOR，本轮修复）：1) publish 门禁改为正式版本白名单（material 仅 material-basic-validation-v2；v1/未知/未来版本伪造 VALIDATED+VERIFIED 也 NOT_READY；PBOC 自有版本不受影响）；2) conflict 判定先于 stale notice（同键异值 age>7=CONFLICT；同键同值 age>7 保留 DUPLICATE 信息；invalid spec/unit+stale 仍 REJECTED）；3) 生产 DailyGroupKey/AggregateGroupKey 加入 canonicalSpecCode（daily/aggregate CSV 新增 canonicalSpecCode 列并保留行级血缘；同 itemId/日期/来源不同 spec=独立 group 绝不混算；旧文件无该列仍可读，spec=null）；4) docs/04 同步 DEC-059（v2=当前正式、v1=历史 superseded）。**最终回归 60 classes/317 tests/0 failures/0 errors/7 skipped**（真实执行；此前 55/298/0/0/10 为 D4-T05 集成历史运行，已被本最终运行 supersede）。
>
> 【Final Narrow Fix + R0 Doc Closure 追加】（2026-08-11，`fix: bind publish validation versions to item type`→`590ee52`→`docs: finalize Day4 stage evidence`）：**CURRENT FINAL RELEASE VERSION-BINDING RULE（当前有效规则，supersede 白名单表述）**——发布门禁按 authoritative monitor-series item/config metadata（`rateKind`）确定业务类型并校验对应允许版本，绝不按客户端声明的 validationVersion 反推类型、绝不用 itemId 字符串硬编码：exchange-rate（PBOC）item 只允许 `pboc-basic-validation-v1`；material item 只允许 `material-basic-validation-v2`；**Material + pboc-basic-validation-v1 = BLOCKED**、**PBOC + material-basic-validation-v2 = BLOCKED**（交叉版本真实测试覆盖）；config 元数据不可解析=fail-closed NOT_READY；H07/H08=`PRESERVED`（纯 config 驱动）。Sol Stage Delta 确认：Item Type Binding/Material Version Isolation/PBOC Version Isolation/Publish Gate/Previous Finding Regression 全部 PASS。**CURRENT FINAL TECHNICAL REGRESSION = 60 classes / 320 tests / 0 failures / 0 errors / 7 skipped**（590ee52 固定，Technical Evidence Basis，未伪造重新执行）；历史运行 55/298/0/0/10、60/311/0/0/7、60/317/0/0/7 保留为历史记录，不再作为当前/最终。Day4 Stage Review=`PENDING_DELTA_REVIEW`、Day 4=`NOT_COMPLETE`。
...
## 4. Regression（真实执行，Java 17.0.19）
