...
| 亚洲金属网（Asian Metal）`https://www.asianmetal.com.cn/` | 正常公开 HTTPS 握手失败（TLS/HTTPS handshake failed；Remote host terminated the handshake） | 无法取得事实（未确认） | 未确认（无页面事实） | 未确认 | 未确认 | UNVERIFIED（ACCESS_UNVERIFIED：本次调查证据不足以确认来源能力，不能据此确认存在或不存在公开接口） |
...
- **AZ91D（MAT.AZ91D.AM）**：NO_APPROVED_SOURCE —— 未发现任何免费公开来源呈现 AZ91D 报价（Asian Metal 本次调查未取得公开页面事实，UNVERIFIED，不能批准）。
