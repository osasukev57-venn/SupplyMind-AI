...
- 打开「总览」：先展示人民银行后台采集状态；成功后 EUR/CNY、USD/CNY 自动出现最近业务日中间价。材料项若无合法外部数据则保持“暂无数据”，随后用明确标注的 Manual 模拟数据演示。
...
- 指定商业源（SMM/Asian Metal）自动采集能力为 `N/A_APPROVED_FALLBACK`（无会员授权），按项目方补充以 Manual 路线满足 P0，属于已批准的合规降级而非缺陷。
