...
| C12 | 模型调用通过 SupplyMind `LLMService` application port 解耦；Spring AI 1.1.8 `ChatClient`/`ChatModel` 只存在于 infrastructure adapter，`LocalLLMService`保留同一端口的未来扩展点；业务层不得依赖Spring AI或厂商DTO。 | C. 架构冻结决策 | P0-冻结 | LLM 门面与适配层 | D6-T00；D6-T03 | 使用测试替身和Spring AI adapter执行同一请求契约；切换或降级不影响Agent领域对象、EvidencePack与业务文件。 | 依赖边界检查、契约测试、配置样例、结构化响应 | FROZEN |
...
| C20 | Agent固定流程为意图识别→Spring AI在受控Tool Adapter集合内选择工具→SupplyMind应用层校验并执行既有只读Service→Java确定性指标→SupplyMind EvidencePack→LLM解释→应用层证据核验→结构化报告持久化；模型不得直接执行工具实现。 | C. 架构冻结决策 | P0-冻结 | Agent编排、EvidencePack、报告 | D6-T01；D6-T02；D6-T03；D6-T04；D6-T05 | 跟踪一次Agent请求的工具选择、应用执行、证据引用、模型响应、降级和后端核验。 | Agent trace、EvidencePack、结构化报告、降级报告 | FROZEN |
| C21 | P0 Agent工具精确冻结为series.resolve、history.query、period.metrics、quality.inspect、cost.impact、warning.explain、provenance.trace；仅SupplyMind Tool Adapter可声明`@Tool`或稳定`ToolCallback`，全部READ_ONLY、显式DTO与evidenceRefs，禁止任意文件/HTTP/数据库/配置/回填/预警写入或shell。 | C. 架构冻结决策 | P0-冻结 | Agent只读工具边界 | D6-T01；D6-T02 | 对七个工具执行契约、权限、日期范围、已发布数据Gate和证据字段测试；提示注入不得突破只读范围。 | 工具契约、权限测试、调用日志、AT-AI-002 | FROZEN |
| C22 | LLM不判断数据有效性、不读取未校验raw、不计算均值/成本/风险、不编造来源或无证据归因；只接收SupplyMind EvidencePack中的Java结构化指标和证据摘要，模型记忆、对话历史或工具transcript均不得替代正式证据。 | C. 架构冻结决策 | P0-冻结 | LLM输入策略、报告核验 | D6-T02；D6-T03；D6-T04；D6-T05 | 提示注入和故障测试；数字和引用必须匹配EvidencePack，非法工具请求fail-closed并进入模板降级。 | 提示注入测试、声明核验报告、AT-AI-003、降级报告 | FROZEN |
...
| C35 | Day 6框架基线精确冻结为Java 17、Spring Boot 3.5.15、Spring AI 1.1.8；当前Boot 3.3.6只能经D6-T00升级，禁止Boot 4.x、Spring AI 2.x和预发布版本。D6-T00必须在Day5完整基线上证明全部既有测试、文件合同和业务语义无回归，否则拒绝升级并回退`day5-complete`/`36dc178`。 | C. DEC-060架构变更 | P0-最高升级Gate | dependency、configuration、full regression | D6-T00 | 核对精确依赖树并完整执行Day1～Day5回归；历史基线83 suites/407 tests/0 failures/0 errors/8 skipped，数量变化须逐项解释，禁止核心测试静默skip。 | dependency tree、完整Surefire结果、测试清点、差异报告、rollback证据、AT-AI-000 | FROZEN |
| C36 | Spring AI只替代LLM传输、请求/响应映射和受控tool-calling编排；SupplyMind保有LLMService端口、工具校验/执行、EvidencePack/AgentReport schema、业务计算、证据核验、报告持久化和Java模板降级。缺密钥、timeout、429、5xx、畸形/空响应或非法工具请求不得使核心API整体失败。 | C. DEC-060架构变更 | P0-冻结 | agent/application/infrastructure | D6-T01；D6-T02；D6-T03；D6-T04；D6-T05 | 本地stub完成工具选择/合同/降级验收；云端真实调用单独gated且无密钥时保持NOT_RUN/BLOCKED，不得伪造PASS。 | AGENT-EVIDENCE-SCHEMA-V1、AT-AI-001/002/003、EvidencePack、AgentReport、故障注入报告 | FROZEN |
...
| EXT-10 | 获认可的免费公开材料信源、许可条款、更新频率和字段映射。 | F/C. DEC-063 部分决议 | 免费源能力/非全局阻塞 | FreePublic、provenance | D3-T03；D3-T06；Post-Day10 | ADC12 使用 SHFE 铸造铝合金期货结算价公开基准；AZ91D 未确认时 Manual 保底。 | URL、完整 raw/SHA、字段映射、解析/全链测试 | PARTIALLY_RESOLVED_NON_BLOCKING |
...
| EXT-10 | 免费公开信源认可与字段映射 | DEC-063 已接受 SHFE 铸造铝合金期货结算价作为 ADC12 同类公开基准；AZ91D 仍待确认。 | 来源失效或语义不等价。 | Post-Day10 | SHFE fail-closed；AZ91D 用 Manual/LocalImport；均不能冒充商业源。 | 否，不阻塞整体P0。 |
